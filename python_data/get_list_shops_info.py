...
result = {}
for shopid in tqdm(zip(p_idshop), total=len(p_idshop)):
    shop_id = int(shopid[0])
    response = requests.get('https://shopee.vn/api/v4/product/get_shop_info?shopid={}'.format(shop_id), headers=headers)
    if response.status_code == 200:
        print('Crawl data {} success !!!'.format(shopid))
        result['{}'.format(shopid)] = response.json()
    # time.sleep(random.randrange(3, 5))

with open('list_shops_info.json', 'w') as file:
