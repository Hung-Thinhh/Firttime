    ...
    function Validate(inputElement, rule) {
        ...
        if (errorMessage) {
            ...
            btn_login.onclick = function (e) {
                ...
                const url = '/api/login'; // đường dẫn tới endpoint xử lý đăng nhập trên server
                const data = { // thông tin đăng nhập
                ...
                xhr.onreadystatechange = function () {
                    if (this.readyState === 4 && this.status === 200) {
                        ...
                        console.log(response); // xử lý kết quả trả về từ server
                        if (response.errCode == 0) {
                            document.querySelector('.form-erro').classList.add('displayed')

                            console.log("Login ok")
                            setCookie('token', response.token, 1)
                            localStorage.setItem('is_from_login', true);
                            window.location.href = '/is_from_login=true';
                        }
                        else {
                            document.querySelector('.form-erro').classList.remove('displayed')
                            document.querySelectorAll('.group1')[0].classList.add('invalid')
                            document.querySelectorAll('.group1')[1].classList.add('invalid')
                            localStorage.setItem('is_from_login', JSON.stringify({
                                is_from_login: false
                            }));
