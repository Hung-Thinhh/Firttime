...
for pid, shopid in tqdm(zip(p_ids, p_idshop), total=len(p_ids)):
    response = requests.get('https://shopee.vn/api/v4/item/get?itemid={}&shopid={}'.format(pid,shopid), headers=headers)
    if response.status_code == 200:
        print('Crawl data {} success !!!'.format(pid))
        result['{}_{}'.format(pid,shopid)] = response.json()
    # time.sleep(random.randrange(3, 5))

with open('list_infomation_recomment_item.json', 'w') as file:
