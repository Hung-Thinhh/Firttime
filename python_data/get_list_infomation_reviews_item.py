...
df_id = pd.read_csv('product_id_recomment_ncds.csv')
p_ids = df_id.iditem.to_list()
p_idshop = df_id.idshop.to_list()

result = {}
for pid, shopid in tqdm(zip(p_ids, p_idshop), total=len(p_ids)):
    response = requests.get('https://shopee.vn/api/v2/item/get_ratings?filter=0&flag=1&itemid={}&limit=6&offset=0&shopid={}&type=0'.format(pid,shopid), headers=headers)
    if response.status_code == 200:
        print('Crawl data {} success !!!'.format(pid))
        result['{}_{}'.format(pid,shopid)] = response.json()
    # time.sleep(random.randrange(3, 5))

with open('list_infomation_reviews_item.json', 'w') as file:
    file.write(json.dumps(result, indent=5))