...
axios.get('/api/listRecommend', {
    ...
    // console.log(page)
        ...
        console.log(shops.data.sections[0].data.item.length)
        for(let i =0 ; i < item_recomment_img.length; i++){
            if(i >= shops.data.sections[0].data.item.length){
                items[i].style.display = 'none'
                console.log(113)
            }else{
                
                item_recomment_img[i].src = `https://cf.shopee.vn/file/${shops.data.sections[0].data.item[i].image}`

            }
        }
        for(let i =0 ; i < item_recomment_name.length; i++){
            if(i < shops.data.sections[0].data.item.length){
                item_recomment_name[i].innerHTML = shops.data.sections[0].data.item[i].name
            }
        }
        ...
        for(let i =0 ; i < item_price.length; i++){
            if(i < shops.data.sections[0].data.item.length){
                    ...
                    item_price[i].querySelector('.container__main-category-recommed-panels-item-botton-cost-name2').innerHTML = `${(shops.data.sections[0].data.item[i].price_min/100000).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}`
        ...
        for(let i =0 ; i < item_sold.length; i++){
            if(i < shops.data.sections[0].data.item.length){
                if(i < shops.data.sections[0].data.item.length){
                    const sold = shops.data.sections[0].data.item[i].historical_sold
                    if (sold/1000 > 1){
        ...
        for(let i =0 ; i < item_discount.length; i++){
            if(i < shops.data.sections[0].data.item.length){
                if(shops.data.sections[0].data.item[i].show_discount == 0){
                    item_discount[i].parentElement.parentElement.parentElement.style.display = 'none'
                }
                else{
                    item_discount[i].innerHTML = `${shops.data.sections[0].data.item[i].show_discount}%`
                }
            }
        }
        ...
        for(let i =0 ; i < item_liked_label.length; i++){
            if(i < shops.data.sections[0].data.item.length){
                if(shops.data.sections[0].data.item[i].show_shopee_verified_label == false){
        ...
        for(let i =0 ; i < item_location.length; i++){
            if(i < shops.data.sections[0].data.item.length){
                if(shops.data.sections[0].data.item[i].shop_location !== ''){
                    item_location[i].innerHTML = shops.data.sections[0].data.item[i].shop_location
                }
            }
...
// const item_recomment = './json/list_recomment_item.json'
